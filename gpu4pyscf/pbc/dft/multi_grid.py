    ...
    for grids_dense, grids_sparse in tasks:
        ...
        if grids_sparse is None:
            ...
            granularized_mesh_size = cp.asarray(
                cp.ceil(mesh / blocking_sizes_on_gpu), dtype=cp.int32).get()
            ...
            n_primitive_gtos_in_dense = multigrid._pgto_shells(
                subcell_in_dense_region)
            n_primitive_gtos_in_two_regions = multigrid._pgto_shells(
                grouped_cell)
            ...
            for i_angular in set(grouped_cell._bas[0:n_primitive_gtos_in_dense,
                                 multigrid.ANG_OF]):
                i_shells = np.where(
                    grouped_cell._bas[0:n_primitive_gtos_in_dense,
                ...
                for j_angular in set(
                        grouped_cell._bas[0:n_primitive_gtos_in_two_regions,
                        multigrid.ANG_OF]):
                    j_shells = np.where(
                        grouped_cell._bas[0:n_primitive_gtos_in_two_regions,
                    ...
                    j_coeffs = cp.asarray(
                        grouped_cell._env[j_basis[:,
                                          multigrid.PTR_COEFF]]) * CINTcommon_fac_sp(
                        j_angular)
                    pair_exponents = cp.add.outer(i_exponents,
                                                  j_exponents).flatten()
                    ...
                    for image_index, i_image in enumerate(
                        ...
                        selected_pair_exponents = pair_exponents[
                            non_trivial_pairs]
                        ...
                        selected_pair_exponents = selected_pair_exponents[
                            non_trivial_cutoffs]
                        pair_x = cp.add.outer(i_exponents * shifted_i_x,
                                              j_exponents * j_x).flatten()[
                            non_trivial_pairs]
                        pair_y = cp.add.outer(i_exponents * shifted_i_y,
                                              j_exponents * j_y).flatten()[
                            non_trivial_pairs]
                        pair_z = cp.add.outer(i_exponents * shifted_i_z,
                                              j_exponents * j_z).flatten()[
                            non_trivial_pairs]
                        centers_in_fractional = reciprocal_lattice_vectors.dot(
                            cp.vstack((pair_x, pair_y,
                                       pair_z)) / selected_pair_exponents)

                        cutoffs_in_fractional = cp.outer(reciprocal_norms,
                                                         gaussian_cutoffs)
                        ...
                        broad_enough_pairs = \
                            cp.where(cp.all(begin <= end, axis=1))[0]
                        non_trivial_pairs = non_trivial_pairs[
                            broad_enough_pairs]
                    ...
                    contributing_area_begin = cp.asarray(
                        cp.concatenate(contributing_area_begin),
                        dtype=cp.int32).T
                    contributing_area_end = cp.asarray(
                        cp.concatenate(contributing_area_end), dtype=cp.int32).T
                    bin = cp.array([8, 8, 8])
                    binned_ranges = cp.asarray(cp.ceil((contributing_area_end - contributing_area_begin).T / bin),
                                               dtype=cp.int32) * bin
                    binned_ranges = binned_ranges.get()
                    indices = []
                    unique = np.unique(binned_ranges, axis=0)
                    for unique_range in unique:
                        indices.append(cp.asarray(np.where(
                            np.all(binned_ranges == unique_range, axis=1))[0],
                                                  dtype=cp.int32))
                    ...
                    for xyz_index in range(3):
                        ...
                        begin_translation = cp.asarray(
                            cp.ceil(cp.subtract.outer(
                                cp.arange(local_grid_size) * blocking_sizes[
                                    xyz_index],
                                contributing_area_end[xyz_index]) /
                                    mesh[xyz_index]), dtype=cp.int32)
                        end_translation = cp.asarray(cp.floor(cp.subtract.outer(
                            cp.arange(local_grid_size) * blocking_sizes[
                                xyz_index],
                            contributing_area_begin[xyz_index]) / mesh[
                                                                  xyz_index]),
                                                     dtype=cp.int32)
                        contributing = (
                                begin_translation <= end_translation).T.get()
                        contributing_indices.append(list(
                            map(lambda bools: np.where(bools)[0],
                                contributing)))
                    ...
                    for i_pair in range(n_pairs):
                        contributing_local_grid = np.add.outer(
                            ...
                            granularized_mesh_size[1] * granularized_mesh_size[
                                2],
                            np.add.outer(contributing_indices[1][i_pair] *
                    ...
                    pair_indices = cp.concatenate(
                        [cp.full(len(contributing_indices_for_pairs[i]), i) for
                         i in range(n_pairs)])
                    ...
                    contributing_indices_for_pairs = \
                        contributing_indices_for_pairs[sort_indices]
                    ...
                    effective_local_grid_points, counts = np.unique(
                        contributing_indices_for_pairs.get(),
                        return_counts=True)
                    n_pairs_per_point = cp.zeros(
                        cp.prod(granularized_mesh_size) + 1)
                    n_pairs_per_point[effective_local_grid_points + 1] = counts
                    accumulated_n_pairs_per_point = cp.asarray(
                        cp.ufunc.accumulate(cp.add, n_pairs_per_point),
                        dtype=cp.int32)
...
def evaluate_density_wrapper(pairs_info, dm_slice, ignore_imag=True):
    ...
    if n_k_points == 0:
        ...
        density_matrix_with_translation = cp.einsum("kt, ikpq->itpq",
                                                    pairs_info[
                                                        "phase_diff_among_images"],
                                                    dm_slice)
...
def evaluate_density_diffused_wrapper(pairs_info, dm_slice, ignore_imag=True):
    ...
    for gaussians_per_angular_pair in pairs_info["per_angular_pairs"]:
        for blocking_size, corresponding_indices in zip(
                gaussians_per_angular_pair["block_sizes"],
                gaussians_per_angular_pair["indices_with_same_block"]):
            selected_pairs = gaussians_per_angular_pair["non_trivial_pairs"][
                corresponding_indices]
            selected_image_indices = \
                gaussians_per_angular_pair["image_indices"][selected_pairs]
            selected_pairs_area_begin = gaussians_per_angular_pair[
                                            "contributing_area_begin"][:,
                                        selected_pairs]
            ...
            c_driver(ctypes.cast(density.data.ptr, ctypes.c_void_p),
                     ctypes.cast(
                         density_matrix_with_translation_real_part.data.ptr,
                         ctypes.c_void_p),
                     ctypes.c_int(i_angular), ctypes.c_int(j_angular),
                     ctypes.cast(selected_pairs.data.ptr, ctypes.c_void_p),
                     ctypes.c_int(len(selected_pairs)),
                     ctypes.cast(
                         gaussians_per_angular_pair["i_shells"].data.ptr,
                         ctypes.c_void_p),
                     ctypes.cast(
                         gaussians_per_angular_pair["j_shells"].data.ptr,
                         ctypes.c_void_p),
                     ctypes.c_int(len(gaussians_per_angular_pair["j_shells"])),
                     ctypes.cast(gaussians_per_angular_pair[
                                     "shell_to_ao_indices"].data.ptr,
                                 ctypes.c_void_p),
                     ctypes.c_int(n_i_functions), ctypes.c_int(n_j_functions),
                     ctypes.cast(selected_pairs_area_begin.data.ptr,
                                 ctypes.c_void_p),
                     ctypes.cast(selected_image_indices.data.ptr,
                                 ctypes.c_void_p),
                     ctypes.cast(pairs_info["neighboring_images"].data.ptr,
                                 ctypes.c_void_p), ctypes.c_int(n_images),
                     ctypes.cast(pairs_info["lattice_vectors"].data.ptr,
                                 ctypes.c_void_p),
                     ctypes.cast(
                         pairs_info["reciprocal_lattice_vectors"].data.ptr,
                         ctypes.c_void_p),
                     (ctypes.c_int * 3)(*pairs_info["mesh"]),
                     ctypes.cast(pairs_info["atm"].data.ptr, ctypes.c_void_p),
                     ctypes.cast(pairs_info["bas"].data.ptr, ctypes.c_void_p),
                     ctypes.cast(pairs_info["env"].data.ptr, ctypes.c_void_p),
                     (ctypes.c_int * 3)(*blocking_size),
                     ctypes.c_int(n_channels))
...
def evaluate_density_on_g_mesh(mydf, dm_kpts, hermi=1, kpts=np.zeros((1, 3)),
    ...
    for (grids_dense, grids_sparse), pairs in zip(tasks,
        ...
        if grids_sparse is None:
            ...
            if deriv == 0:
                n_ao_in_sparse, n_ao_in_dense = density_matrix_with_rows_in_sparse.shape[
                                                2:]
                density_matrix_with_rows_in_dense[:, :, :,
                n_ao_in_dense:] += density_matrix_with_rows_in_sparse.transpose(
                    0, 1, 3, 2)
                ...
                density = evaluate_density_wrapper(pairs,
                                                   coeff_sandwiched_density_matrix)
            else:
                raise NotImplementedError
...
def convert_xc_on_g_mesh_to_fock(mydf, xc_on_g_mesh, hermi=1,
    ...
    for (grids_dense, grids_sparse), pairs in zip(mydf.tasks,
                                                  mydf.sorted_gaussian_pairs):
        mesh = pairs["mesh"]
        n_grid_points = np.prod(mesh)
        ...
        if grids_sparse is None:
            ...
            for ao_on_sliced_grid_in_dense, p0, p1 in mydf.aoR_loop(grids_dense,
                                                                    kpts):
                ...
                for k in range(n_k_points):
                    for i in range(n_channels):
                        xc_scaled_ao = numint._scale_ao(ao_values[k],
                                                        ...
                                                        i, p0:p1])
                        xc_sub_block = cp.dot(ao_values[k].conj().T,
                                              xc_scaled_ao)
                        fock[i, k, ao_index_in_dense[:,
                                   None], ao_index_in_dense] += xc_sub_block
            ...
            fock_slice = cp.einsum('nkpq,pi->nkiq', fock_slice,
                                   pairs["coeff_in_dense"])
            fock_slice = cp.einsum('nkiq,qj->nkij', fock_slice,
                                   pairs["concatenated_coeff"])

            fock[:, :, pairs["ao_indices_in_dense"][:, None],
            pairs["ao_indices_in_dense"]] += fock_slice[:, :, :,
                                             :n_ao_in_sparse]
            fock[:, :, pairs["ao_indices_in_dense"][:, None],
            pairs["ao_indices_in_sparse"]] += fock_slice[:, :, :,
                                              n_ao_in_sparse:]

            if hermi == 1:
                fock[:, :, pairs["ao_indices_in_sparse"][:, None],
                pairs["ao_indices_in_dense"]] += \
                    fock_slice[:, :, :, n_ao_in_sparse:].transpose(0, 1, 3,
                                                                   2).conj()
            else:
                raise NotImplementedError
