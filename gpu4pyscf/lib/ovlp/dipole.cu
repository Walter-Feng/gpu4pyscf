...
__global__ void
dipole_kernel(double *dipole, const int *pair_indices, const int n_primitives,
  ...
  dipole += blockIdx.y * 3 * n_functions * n_functions +
  ...
  write_integral<i_angular, j_angular, j_angular + 2>(dipole, x_pairs, y_pairs,
                                                      z_pairs, n_functions);
  rr::fill_with_recursion<i_angular, j_angular + 1>(
      x_pairs, prefactor, factor_a * ix_to_jx, factor_b, ix_to_jx);

  dipole += n_functions * n_functions;
...
#define dipole_kernel_macro(i, j)                                              \
  case i * 10 + j:                                                             \
    ovlp::dipole_kernel<i, j><<<block_grid, block_size>>>(                     \
        dipole, pair_indices, n_primitives, n_pairs, primitive_to_function,    \
        n_functions, atm, atm_stride, bas, bas_stride, env, env_stride,        \
        reference_point_x, reference_point_y, reference_point_z);              \
    break;
extern "C" {
void dipole(double *dipole, const int *pair_indices, const int n_pairs,
  ...
  switch (i_angular * 10 + j_angular) {
    dipole_kernel_macro(0, 0);
    dipole_kernel_macro(0, 1);
    dipole_kernel_macro(0, 2);
    dipole_kernel_macro(0, 3);
    dipole_kernel_macro(0, 4);

    dipole_kernel_macro(1, 0);
    dipole_kernel_macro(1, 1);
    dipole_kernel_macro(1, 2);
    dipole_kernel_macro(1, 3);
    dipole_kernel_macro(1, 4);

    dipole_kernel_macro(2, 0);
    dipole_kernel_macro(2, 1);
    dipole_kernel_macro(2, 2);
    dipole_kernel_macro(2, 3);
    dipole_kernel_macro(2, 4);

    dipole_kernel_macro(3, 0);
    dipole_kernel_macro(3, 1);
    dipole_kernel_macro(3, 2);
    dipole_kernel_macro(3, 3);
    dipole_kernel_macro(3, 4);

    dipole_kernel_macro(4, 0);
    dipole_kernel_macro(4, 1);
    dipole_kernel_macro(4, 2);
    dipole_kernel_macro(4, 3);
    dipole_kernel_macro(4, 4);
  }
}
}
