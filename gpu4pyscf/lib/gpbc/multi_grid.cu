...
__inline__ __device__ static void cart2sph_in_place(T g_cart[]) {
  T g_sph[2 * ANG + 1];
...
__global__ void evaluate_gto_gamma_point_kernel(
    double *result, const int *shells, const int n_shells,
    const double *vectors_to_neighboring_images, const int n_images,
    const double *lattice_vectors, const int mesh_a, const int mesh_b,
    const int mesh_c, const int *atm, const int *bas, const double *env) {
  ...
  constexpr int n_cartesian_functions = (angular + 1) * (angular + 2) / 2;
  constexpr int n_spherical_functions = 2 * angular + 1;
  double i_cartesian[n_cartesian_functions];
  double value = 0;

  for (int shell_index = 0; shell_index < n_shells; shell_index++) {
    const int i_shell = shells[shell_index];
    ...
    for (int i_image = 0; i_image < n_images; i_image++) {
      ...
      const double x = position_x - i_x - image_x;
      const double y = position_y - i_y - image_y;
      const double z = position_z - i_z - image_z;
      gto_cartesian<angular>(i_cartesian, x, y, z);
      cart2sph_in_place<double, angular>(i_cartesian);
      const double r_squared = distance_squared(x, y, z);
      const double gaussian =
          i_coeff * common_fac_sp<angular>() * exp(-i_exponent * r_squared);
#pragma unroll
      for (int i_function_index = 0; i_function_index < n_spherical_functions;
           i_function_index++) {
        value += i_cartesian[i_function_index] * gaussian;
      }
    }
    atomicAdd(result + a_index * mesh_b * mesh_c + b_index * mesh_c + c_index,
              value);
  }
}

template <int angular>
__global__ void evaluate_gto_k_point_kernel(
    double *result_real, double *result_imag, const int *shells,
    const int n_shells, const double *vectors_to_neighboring_images,
    const int n_images, const double *k_points, const int n_k_points,
    const double *lattice_vectors, const int mesh_a, const int mesh_b,
    const int mesh_c, const int *atm, const int *bas, const double *env) {
  const uint a_index = threadIdx.z + blockDim.z * blockIdx.z;
  const uint b_index = threadIdx.y + blockDim.y * blockIdx.y;
  ...
  const int k_point_stride = mesh_a * mesh_b * mesh_c;
  const uint index = a_index * mesh_b * mesh_c + b_index * mesh_c + c_index;
  result_real += index;
  result_imag += index;
  ...
  for (int shell_index = 0; shell_index < n_shells; shell_index++) {
    ...
    for (int k_point_index = 0; k_point_index < n_k_points; k_point_index++) {
      double real_value = 0;
      double imag_value = 0;
      ...
      for (int i_image = 0; i_image < n_images; i_image++) {
        ...
        const double gaussian =
            i_coeff * common_fac_sp<angular>() * exp(-i_exponent * r_squared);
        const double phase = k_points[k_point_index * 3] * image_x +
                             k_points[k_point_index * 3 + 1] * image_y +
                             k_points[k_point_index * 3 + 2] * image_z;

        const double phase_real = cos(phase);
        const double phase_imag = sin(phase);

#pragma unroll
        for (int i_function_index = 0; i_function_index < n_spherical_functions;
             i_function_index++) {
          real_value += i_cartesian[i_function_index] * gaussian * phase_real;
          imag_value += i_cartesian[i_function_index] * gaussian * phase_imag;
        }
      }
      atomicAdd(result_real + k_point_index * k_point_stride, real_value);
      atomicAdd(result_imag + k_point_index * k_point_stride, imag_value);
    }
  }
}

template <bool is_gamma_point>
void evaluate_gto_driver(double *result_real, double *result_imag,
                         const int angular, const int *shells,
                         const int n_shells,
                         const double *vectors_to_neighboring_images,
                         const int n_images, const double *k_points,
                         const int n_k_points, const double *lattice_vectors,
                         const int mesh[3], const int *atm, const int *bas,
                         const double *env, const int blocking_sizes[3]) {
  ...
  if constexpr (is_gamma_point) {
    switch (angular) {
    case 0:
      evaluate_gto_gamma_point_kernel<0><<<block_grid, block_size>>>(
          result_real, shells, n_shells, vectors_to_neighboring_images,
          n_images, lattice_vectors, mesh_a, mesh_b, mesh_c, atm, bas, env);
      break;
    case 1:
      evaluate_gto_gamma_point_kernel<1><<<block_grid, block_size>>>(
          result_real, shells, n_shells, vectors_to_neighboring_images,
          n_images, lattice_vectors, mesh_a, mesh_b, mesh_c, atm, bas, env);
      break;
    case 2:
      evaluate_gto_gamma_point_kernel<2><<<block_grid, block_size>>>(
          result_real, shells, n_shells, vectors_to_neighboring_images,
          n_images, lattice_vectors, mesh_a, mesh_b, mesh_c, atm, bas, env);
      break;
    case 3:
      evaluate_gto_gamma_point_kernel<3><<<block_grid, block_size>>>(
          result_real, shells, n_shells, vectors_to_neighboring_images,
          n_images, lattice_vectors, mesh_a, mesh_b, mesh_c, atm, bas, env);
      break;
    case 4:
      evaluate_gto_gamma_point_kernel<4><<<block_grid, block_size>>>(
          result_real, shells, n_shells, vectors_to_neighboring_images,
          n_images, lattice_vectors, mesh_a, mesh_b, mesh_c, atm, bas, env);
      break;
    default:
      fprintf(stderr,
              "angular momentum %d is not supported in evaluate_gto_driver\n",
              angular);
    }
    ...
    switch (angular) {
    case 0:
      evaluate_gto_k_point_kernel<0><<<block_grid, block_size>>>(
          result_real, result_imag, shells, n_shells,
          vectors_to_neighboring_images, n_images, k_points, n_k_points,
          lattice_vectors, mesh_a, mesh_b, mesh_c, atm, bas, env);
      break;
    case 1:
      evaluate_gto_k_point_kernel<1><<<block_grid, block_size>>>(
          result_real, result_imag, shells, n_shells,
          vectors_to_neighboring_images, n_images, k_points, n_k_points,
          lattice_vectors, mesh_a, mesh_b, mesh_c, atm, bas, env);
      break;
    case 2:
      evaluate_gto_k_point_kernel<2><<<block_grid, block_size>>>(
          result_real, result_imag, shells, n_shells,
          vectors_to_neighboring_images, n_images, k_points, n_k_points,
          lattice_vectors, mesh_a, mesh_b, mesh_c, atm, bas, env);
      break;
    case 3:
      evaluate_gto_k_point_kernel<3><<<block_grid, block_size>>>(
          result_real, result_imag, shells, n_shells,
          vectors_to_neighboring_images, n_images, k_points, n_k_points,
          lattice_vectors, mesh_a, mesh_b, mesh_c, atm, bas, env);
      break;
    case 4:
      evaluate_gto_k_point_kernel<4><<<block_grid, block_size>>>(
          result_real, result_imag, shells, n_shells,
          vectors_to_neighboring_images, n_images, k_points, n_k_points,
          lattice_vectors, mesh_a, mesh_b, mesh_c, atm, bas, env);
      break;
...
void evaluate_gto_driver(double *result_real, double *result_imag,
                         ...
                         const int mesh[3], const int *atm, const int *bas,
                         const double *env, const int blocking_sizes[3],
                         const bool is_gamma_point) {
  if (is_gamma_point) {
    evaluate_gto_driver<true>(result_real, result_imag, angular, shells,
                              n_shells, vectors_to_neighboring_images, n_images,
                              k_points, n_k_points, lattice_vectors, mesh, atm,
                              bas, env, blocking_sizes);
  } else {
    evaluate_gto_driver<false>(result_real, result_imag, angular, shells,
                               n_shells, vectors_to_neighboring_images,
                               n_images, k_points, n_k_points, lattice_vectors,
                               mesh, atm, bas, env, blocking_sizes);
  }
}
