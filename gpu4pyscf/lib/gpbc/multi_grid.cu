...
template <int ANG>
__device__ static void _memset_cart(double *g_cart, int stride, int grid_id) {
  if (ANG == 0) {
    g_cart[grid_id] = 0.0;
  } else if (ANG == 1) {
    g_cart[grid_id] = 0.0;
...
template <int ANG>
__device__ static void _memset_sph(double *g_sph, int stride, int grid_id) {
  if (ANG == 0) {
    g_sph[grid_id] = 0.0;
  } else if (ANG == 1) {
    g_sph[grid_id] = 0.0;
...
template <int ANG>
__device__ static void _cart_gto(double *g, double fx, double fy, double fz) {
...
__global__ void evaluate_density_kernel(
  ...
  double
      prefactor[n_channels * n_i_spherical_functions * n_j_spherical_functions];
...
#pragma unroll
    for (int i_channel = 0; i_channel < n_channels; i_channel++) {
      for (int i_function_index = 0; i_function_index < n_i_spherical_functions;
           i_function_index++) {
        for (int j_function_index = 0;
          ...
          prefactor[i_channel * n_i_spherical_functions *
                        n_j_spherical_functions +
                    i_function_index * n_j_spherical_functions +
...
#pragma unroll
          for (int i_channel = 0; i_channel < n_channels; i_channel++) {
            for (int i_function_index = 0;
                 i_function_index < n_i_spherical_functions;
                 i_function_index++) {
              for (int j_function_index = 0;
                   j_function_index < n_j_spherical_functions;
                   j_function_index++) {
                density_value[i_channel] +=
                    prefactor[i_channel * n_i_spherical_functions *
                              ...
                              i_function_index * n_j_spherical_functions +
                              j_function_index] *
                    gaussian * i_spherical[i_function_index] *
                    j_spherical[j_function_index];
              }
            }
          }
        }
...
#define density_kernel_macro(li, lj)                                           \
  evaluate_density_kernel<n_channels, li, lj><<<block_grid, block_size>>>(     \
      ...
      n_i_shells, j_shells, n_j_shells, shell_to_ao_indices, n_i_functions,    \
...
void evaluate_density_driver(
    double *density, const double *density_matrices, const int left_angular,
    const int right_angular, const int *non_trivial_pairs,
    const double *cutoffs, const int *i_shells, const int n_i_shells,
...
__global__ void evaluate_xc_kernel(
  ...
  double neighboring_gaussian_sum[n_i_spherical_functions *
                                  n_j_spherical_functions];
...
#pragma unroll
    for (int i_function_index = 0; i_function_index < n_i_spherical_functions;
         i_function_index++) {
      for (int j_function_index = 0; j_function_index < n_j_spherical_functions;
           j_function_index++) {
        neighboring_gaussian_sum[i_function_index * n_j_spherical_functions +
                                 j_function_index] = 0;
      }
    }
    for (int a_cell = lower_a_index; a_cell <= upper_a_index; a_cell++) {
...
#pragma unroll
          for (int i_function_index = 0;
               i_function_index < n_i_spherical_functions; i_function_index++) {
            for (int j_function_index = 0;
                 j_function_index < n_j_spherical_functions;
                 j_function_index++) {
...
#define xc_kernel_macro(li, lj)                                                \
  evaluate_xc_kernel<n_channels, li, lj><<<block_grid, block_size>>>(          \
      fock, xc_weights, non_trivial_pairs, cutoffs, i_shells, n_i_shells,      \
...
void evaluate_xc_driver(
    double *fock, const double *xc_weights, const int left_angular,
    const int right_angular, const int *non_trivial_pairs,
    const double *cutoffs, const int *i_shells, const int n_i_shells,
    const int *j_shells, const int n_j_shells, const int *shell_to_ao_indices,
    const int n_i_functions, const int n_j_functions,
...
void evaluate_density_driver(
  ...
  if (n_channels == 1) {
    evaluate_density_driver<1>(
        density, density_matrices, left_angular, right_angular,
        non_trivial_pairs, cutoffs, i_shells, n_i_shells, j_shells, n_j_shells,
...
void evaluate_xc_driver(
  ...
  if (n_channels == 1) {
    evaluate_xc_driver<1>(
        fock, xc_weights, left_angular, right_angular, non_trivial_pairs,
        cutoffs, i_shells, n_i_shells, j_shells, n_j_shells,
        shell_to_ao_indices, n_i_functions, n_j_functions,
        sorted_pairs_per_local_grid, accumulated_n_pairs_per_local_grid,
        image_indices, vectors_to_neighboring_images, n_images, lattice_vectors,
        reciprocal_lattice_vectors, mesh, atm, bas, env, blocking_sizes);
