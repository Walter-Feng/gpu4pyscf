...
class Cell(qmmm.mm_mole.Mole, pbc.gto.Cell):
    ...
    def get_ewald_potential(self, coords, images = None):
        ...
        dist2 = contract('jx,jx->j', dist2, dist2)
        n_atoms = len(coords)
        matrix_shape = (n_atoms, n_atoms)

        ewovrl00 = cp.zeros(matrix_shape)
        ewovrl01 = cp.zeros(matrix_shape + (3,))
        ewovrl11 = cp.zeros(matrix_shape + (3, 3))
        ewovrl02 = cp.zeros(matrix_shape + (3, 3))
        ewself00 = cp.zeros(matrix_shape)
        ewself01 = cp.zeros(matrix_shape + (3,))
        ewself11 = cp.zeros(matrix_shape + (3, 3))
        ...
        for i0, i1 in lib.prange(0, len(coords1), blksize):
            ...
            # substract the real-space Coulomb within reference cell 
    ...
    def get_ewald_pot(self, coords1, coords2=None, charges2=None, zetas2=None, xxx=None):
        # breakpoint()
        assert self.dimension == 3
